...
M3 自主根因调查循环（自研 ReAct）。**权威设计**：`docs/design/m3-investigation-loop-design.md`（status: **draft**，2026-09-08 草案完成，G1–G9 开放点待用户评审拍板，依据见该文档「评审依据」R1–R9）。

> ⚠️ **设计票边界**：本 tracker 随设计草案一并建立（建完即止），**G1–G9 全部定案前不派工任何实现票**；定案后 T5/T6 就绪态需终审（可能转 `ready-for-agent`）。
...
| 05 | Verifier（规则层 + LLM 裁决接缝） | T5 | 03, 04 | **ready-for-human**（Verifier 形态 G5 待定案） |
| 06 | 主循环 Loop（终止三出口/失败六值归类/防绕圈四机制） | T6 | 01–05 | **ready-for-human**（主循环结构） |
...
- [ ] G1–G9 评审拍板（**阻塞派工**，见设计文档 §开放设计点；定案后登记 D-22+、新术语入 CONTEXT、设计文档翻 reviewed）
- [ ] M3-A 内核：01–03
- [ ] M3-B 编排：04–06
