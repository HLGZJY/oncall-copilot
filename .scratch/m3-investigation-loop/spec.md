...
M3 自主根因调查循环（自研 ReAct）。**权威设计**：`docs/design/m3-investigation-loop-design.md`（status: **implemented**，2026-09-08 T8 收官实测回填；评审定案与依据见该文档「评审依据」R1–R9）。
...
| 08 | 端到端 3 剧本验证 + 真实实测回填收尾 | T8 | 07 | resolved（2026-09-08，M3 收官） |
...
- [x] M3-C 入口与验收：07 [x]（2026-09-08）· 08 [x]（2026-09-08，M3 收官——mock 3/3 命中；真实实测 Top-1 首版 0/3 如实回填，两个结构缺口注记 issue 08 待 M4）
- [x] 设计文档翻 `implemented`（T8 实测回填后，2026-09-08）
