...
| 上下文管理器 / ContextManager | M3 调查循环的上下文预算组件：摘要模板固定（组件/指标/异常方向/时间窗）、工具输出 ≤2000 tokens 截断 + `[truncated, full at step N]` 指针、系统提示 ≤1500 tokens、步数 ≥10 移出已证伪假设——**视图层过滤**，session 全量保留（D-25） | 上下文组装器、窗口管理器 |
...
| 事件锚点 / Event Anchors | Planner 决策视图 `opening.event_anchors` 携带的**告警时间线**（`{runs:[started_at/recovered_at], alerts:[alert_name/labels/fired_at/resolved_at]}`，形态复用 `eval/evidence.py` timeline 证据面，D-18 同源零标注泄漏）：runner 由 golden 时间线构建注入，消除「只有拓扑没有事件」的盲调查态（M3 issue 09，兑现 M5 issue 08 注记）——与 D-37 `opening` 键（D-17 卡片投影）同键共存不冲突 | 告警时间线注入、事件时间轴 |
